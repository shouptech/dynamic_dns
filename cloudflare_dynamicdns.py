...
def get_zone_id(dnsname, authemail, authkey):
    ...
    params = {"name": "%s.%s" % tuple(dnsname.split(".")[-2:])}
...
def update_dns(zoneid, recordid, dnsname, ip, authemail, authkey):
    """Update DNS record"""
    headers = {"X-Auth-Email": authemail, "X-Auth-Key": authkey}
    payload = {"type": "A", "name": dnsname, "content": ip}
...
def main():
    ...
    if curdns == None:
        ...
        response = create_dns(zoneid, args.dnsname, myip, args.authemail, args.authkey)
        print(response.json())
        ...
        response = update_dns(
            zoneid, curdns["id"], args.dnsname, myip, args.authemail, args.authkey
        )
        print(response.json())
